<?php 
/* Styles */
add_action( 'wp_enqueue_scripts', 'speakers_outlet_enqueue_styles' );
function speakers_outlet_enqueue_styles() {
	wp_enqueue_style( 'parent-style', get_template_directory_uri() . '/style.css' ); 
} 

/* New fonts */
function speakers_outlet_google_fonts() {
	wp_enqueue_style( 'speakers-outlet-google-fonts', 'http://fonts.googleapis.com/css?family=Lato:Noto+Serif:400,400i&display=swap', false ); 
}
add_action( 'wp_enqueue_scripts', 'speakers_outlet_google_fonts' );
...
/* New excerpt lengths */
function speakers_outlet_custom_excerpt_length( $length ) {
	return 30;
}
add_filter( 'excerpt_length', 'speakers_outlet_custom_excerpt_length', 999 );


/*New customizer function */
function speakers_outlet_customize_register( $wp_customize ) {
	$wp_customize->add_section( 'read_more_button', array(
		'title'      => __('Read More Button','speakers-outlet'),
		'priority'   => 1,
		'capability' => 'edit_theme_options',
		) );

	$wp_customize->add_setting( 'readmorebutton_color', array(
		'default'           => '#333',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'postMessage',
		) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'readmorebutton_color', array(
		'label'       => __( 'Text Color', 'speakers-outlet' ),
		'section'     => 'read_more_button',
		'priority'   => 1,
		'settings'    => 'readmorebutton_color',
		) ) );

	$wp_customize->add_setting( 'readmorebutton_border', array(
		'default'           => '#fab526',
		'sanitize_callback' => 'sanitize_hex_color',
		'transport'         => 'postMessage',
		) );
	$wp_customize->add_control( new WP_Customize_Color_Control( $wp_customize, 'readmorebutton_border', array(
		'label'       => __( 'Border Color', 'speakers-outlet' ),
		'section'     => 'read_more_button',
		'priority'   => 1,
		'settings'    => 'readmorebutton_border',
		) ) );

}
add_action( 'customize_register', 'speakers_outlet_customize_register' );




if(! function_exists('speakers_outlet_customizer_css_final_output' ) ):
	function speakers_outlet_customizer_css_final_output(){
		?>
		...
		<?php }
		add_action( 'wp_head', 'speakers_outlet_customizer_css_final_output' );
		endif;
