...
if($_GET[mode]=="del"){
	sql_query("delete from audition04 where a_num='$_GET[num]'");

?>
<script language='javascript'>
alert('삭제되었습니다');
location.href='page04_list.php?stx=<?=$_GET[stx]?>';
</script>
<?
	exit;
}
...
function del(num,stx)
{
	...
	{
		location.href='page04_list.php?mode=del&num='+num+'&stx='+stx;
	}
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
   ...
   //$info = GetImageSize("../../03_audition/file/$row[a_profile1]");
    ...
    <tr class='list$list col1 ht center'>
        <td align=left>&nbsp;".$s_profile1."&nbsp;<a href='page04_view.php?num=$row[a_num]&stx=$stx'>".$row[a_name]."</a></td>
        <td>$row[a_kind]</td>
