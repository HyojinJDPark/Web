...
if($_GET[mode]=="del"){
	sql_query("delete from audition02 where a_num='$_GET[num]'");

?>
<script language='javascript'>
alert('삭제되었습니다');
location.href='page01_list.php?stx=<?=$_GET[stx]?>';
</script>
<?
	exit;
}
...
function del(num,stx)
{
	...
	{
		location.href='page01_list.php?mode=del&num='+num+'&stx='+stx;
	}
...
for ($i=0; $row=sql_fetch_array($result); $i++) {
	...
	//$s_profile1="<img src='http://blueswallow.kr/03_audition02/file/$row[a_profile1]' width='50' height='50'>";
   //$info = GetImageSize("../../03_audition02/file/$row[a_profile1]");
    echo "
    <tr class='list$list col1 ht center'>
        <td align=left>&nbsp;".$s_profile1."&nbsp;<a href='page01_view.php?num=$row[a_num]&stx=$stx'>".$row[a_name]."</a></td>
        <td>$row[a_h01]</td>
