<?
$menu["menu200"] = array (
    array("200000", "회원관리", ""),
    array("200100", "회원관리", "$g4[admin_path]/member_list.php"),
    array("200200", "포인트관리", "$g4[admin_path]/point_list.php"),
    array("200300", "회원메일발송", "$g4[admin_path]/mail_list.php"),
    array("-"),
    array("200800", "접속자현황", "$g4[admin_path]/visit_list.php"),
    array("-"),
    array("200900", "투표관리", "$g4[admin_path]/poll_list.php"),
     array("201000", "서비스신청현황", "$g4[admin_path]/page01_list.php"),
	  array("201100", "입사원서신청현황", "$g4[admin_path]/page02_list.php"),
	   array("201000", "자원봉사신청현황", "$g4[admin_path]/page03_list.php"),
	    array("201000", "후원신청현황", "$g4[admin_path]/page04_list.php")
	 );
